...
const _ignoreVersions = Symbol.for('ignoreVersions')
...
const _ignoreRange = Symbol('ignoreRange')
...
module.exports = cls => class IdealTreeBuilder extends cls {
  constructor (options) {
    ...
    this[_global] = !!global
    this[_ignoreVersions] = options.ignoreVersions
    console.log("incoming build ideal tree option for ignore versions", options.ignoreVersions)
  ...
  [_ignoreRange] (name) {
    ...
    // make sure only one package is updated a time if ignoring ranges
    // eventually split this into a list for each ignored range
    const ignored = this[_ignoreVersions]
    return ignored
  }
...
`)
  ...
  [_problemEdges] (node) {
    ...
    return [...node.edgesOut.values()]
      .filter(edge => {
        ...
        // user explicitly ignores versions for a package, problem
        if (this[_ignoreRange](edge.to)) {
          return true
        }
  ...
  async [_fetchManifest] (spec) {
    ...
    if (ignored)
      avoid.push(ignored)
    const avoided = this[_avoidRange](spec.name)
    if (avoided)
      avoid.push(avoided)

    console.log("avoided", avoid.flat())
    const options = {
      ...this.options,
      avoid: avoid.flat().toString()
    }
